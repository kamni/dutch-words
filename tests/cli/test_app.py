...
class TestTenThousandSentencesApp(TestCase):
    ...
    async def test_on_mount_app_not_configured(self):
        ...
        async with app.run_test() as pilot:
            expected = app.get_screen('first_time', FirstTimeModal)
            returned = app.screen
    ...
    async def test_on_mount_user_not_logged_in(self):
        ...
        user = make_user_db(is_admin=True)
        ...
        auth_adapter = adapters.get('AuthPort')
        self.assertTrue(auth_adapter.is_configured)

        '''
        app = TenThousandWordsApp()
        async with app.run_test() as pilot:
            expected = app.get_screen('login', LoginScreen)
            returned = app.screen
            self.assertEqual(expected, returned)
